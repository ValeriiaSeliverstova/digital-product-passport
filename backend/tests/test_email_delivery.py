...
from app.email_delivery import send_password_reset_email, send_tracking_email
...
def test_password_reset_email_reuses_smtp_configuration(
    monkeypatch: MonkeyPatch,
) -> None:
    captured: dict[str, object] = {}
    ...
    monkeypatch.setattr(settings, "smtp_password", SecretStr("app-password"))
    monkeypatch.setattr(settings, "smtp_from_email", "example@ukr.net")
    monkeypatch.setattr(settings, "frontend_origin", "https://dpp.example.com")
    monkeypatch.setattr("app.email_delivery.smtplib.SMTP_SSL", FakeSmtp)
    ...
    assert (
        "https://dpp.example.com/reset-password?token=private-reset-token"
